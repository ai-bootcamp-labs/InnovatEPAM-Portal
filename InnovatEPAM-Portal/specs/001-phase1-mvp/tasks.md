---
description: "Task list for Phase 1 MVP â€” InnovatEPAM Portal"
---

# Tasks: Phase 1 MVP â€” InnovatEPAM Portal
...
- **[Story]**: `[US1]`, `[US2]`, `[US3]` â€” maps to user stories from spec.md
...
- [x] T008 [P] Add NuGet packages to the appropriate backend projects per [research.md](./research.md): Api â€” `Microsoft.AspNetCore.Authentication.JwtBearer`, `Swashbuckle.AspNetCore`, `Serilog.AspNetCore`, `Serilog.Sinks.Console`, `FluentValidation.AspNetCore`; Infrastructure â€” `Microsoft.EntityFrameworkCore.Design`, `Npgsql.EntityFrameworkCore.PostgreSQL`, `Microsoft.AspNetCore.Identity.EntityFrameworkCore`, `EFCore.NamingConventions`; UnitTests â€” `xunit`, `FluentAssertions`, `Microsoft.NET.Test.Sdk`; IntegrationTests â€” additionally `Microsoft.AspNetCore.Mvc.Testing`, `Testcontainers.PostgreSql`
...
- [x] T011 [P] Initialize shadcn/ui in `frontend/` (`npx shadcn@latest init`) producing `frontend/components.json`; configure path alias `@/*` â†’ `frontend/src/*` in `frontend/tsconfig.json` and `frontend/vite.config.ts`
...
- [x] T016 [P] Create `docker-compose.dev.yml` at repo root with the PostgreSQL 15 service from [quickstart.md](./quickstart.md) Â§1.2 (named `innovatepam-pg`, env, port mapping)
...
**Purpose**: Stand up the cross-cutting backbone every user story depends on â€” DB context with Identity, auth pipeline, error handling, logging, OpenAPI, frontend providers and routing shell.

**âš ï¸ CRITICAL**: No user story work begins until this phase is done.

### Backend foundation

- [x] T019 Define `IdeaStatus` and `DecisionAction` enums in `backend/src/InnovatEpam.Portal.Domain/Enums/`
- [x] T020 Create `AppUser` and `AppRole` Identity entities in `backend/src/InnovatEpam.Portal.Domain/Identity/AppUser.cs` and `AppRole.cs` (see [data-model.md](./data-model.md) Â§1, Â§2)
...
- [x] T026 Configure RFC 7807 ProblemDetails: register `AddProblemDetails()`, add custom `IExceptionHandler` `DomainExceptionHandler` in `backend/src/InnovatEpam.Portal.Api/ErrorHandling/` mapping `NotFoundException â†’ 404`, `ConflictException â†’ 409`, `ForbiddenException â†’ 403`, `ValidationException â†’ 400` (per R5)
...
- [x] T031 Define `IAttachmentStorage` interface in `backend/src/InnovatEpam.Portal.Application/Storage/IAttachmentStorage.cs` with `SaveAsync(stream, contentType, ct) â†’ StorageKey`, `OpenReadAsync(key, ct)`, `DeleteAsync(key, ct)`
...
- [x] T044 [P] Create `frontend/src/components/state/`: `LoadingState.tsx` (skeleton variants), `EmptyState.tsx` (icon + copy + CTA), `ErrorState.tsx` (message + retry callback), `StatusBadge.tsx` (`IdeaStatus` â†’ color/label) â€” covers Constitution Principle III "four canonical states" requirement
...
## Phase 3: User Story 1 â€” Submitter captures and tracks an idea (Priority: P1) ğŸ¯ MVP
...
- [ ] T048 [P] [US1] Contract test `AuthEndpointsTests` in `backend/tests/InnovatEpam.Portal.IntegrationTests/Auth/AuthEndpointsTests.cs`: `POST /api/v1/auth/register` returns 201 + JWT; duplicate email â†’ 409; weak password â†’ 400 with ProblemDetails errors; `POST /auth/login` happy path â†’ 200; bad password â†’ 401; `POST /auth/logout` â†’ 204; `GET /auth/me` returns the registered user
- [ ] T049 [P] [US1] Contract test `IdeasCreateAndListTests` in `backend/tests/InnovatEpam.Portal.IntegrationTests/Ideas/IdeasCreateAndListTests.cs`: authenticated `POST /api/v1/ideas` with valid payload â†’ 201 + Location header + status `Submitted`; missing fields â†’ 400 with field errors; `GET /api/v1/ideas` returns the new idea with `total = 1`
- [ ] T050 [P] [US1] Contract test `AttachmentUploadTests` in `backend/tests/InnovatEpam.Portal.IntegrationTests/Attachments/AttachmentUploadTests.cs`: `PUT /api/v1/ideas/{id}/attachment` with PDF on a fresh idea â†’ 201 + `AttachmentSummary`; second `PUT` (replacement) â†’ 200 + new `AttachmentSummary`; `.exe` â†’ 415; 11 MB â†’ 413; non-submitter non-admin caller â†’ 403; download returns the same bytes
...
- [ ] T054 [P] [US1] Create `Category` entity in `backend/src/InnovatEpam.Portal.Domain/Categories/Category.cs` (per data-model Â§3) with private setters and a static factory
- [ ] T055 [P] [US1] Create `Idea` aggregate root in `backend/src/InnovatEpam.Portal.Domain/Ideas/Idea.cs` per data-model Â§4: factory `Create(title, description, categoryId, submitterId)`, instance method `TransitionTo(IdeaStatus next, AppUser actor, string? comment)` enforcing the legal-transitions table; raises `ConflictException` on illegal transitions; emits a domain event for status changes
- [ ] T056 [P] [US1] Create `Attachment` entity in `backend/src/InnovatEpam.Portal.Domain/Attachments/Attachment.cs` per data-model Â§5
- [ ] T057 [P] [US1] Create `IdeaStatusHistory` entity in `backend/src/InnovatEpam.Portal.Domain/Ideas/IdeaStatusHistory.cs` per data-model Â§7
- [ ] T058 [US1] Add EF Core configurations for `Category`, `Idea`, `Attachment`, `IdeaStatusHistory` in `backend/src/InnovatEpam.Portal.Infrastructure/Persistence/Configurations/` mapping all CHECK constraints, indexes, FK delete behaviours, and the `xmin` row-version on `Idea`; wire them into `PortalDbContext.OnModelCreating` (depends on T054â€“T057)
...
## Phase 4: User Story 2 â€” Admin reviews and decides on an idea (Priority: P2)
...
- [ ] T078 [P] [US2] Contract test `DecisionsEndpointTests` in `backend/tests/InnovatEpam.Portal.IntegrationTests/Decisions/DecisionsEndpointTests.cs`: as Admin â€” `MoveToUnderReview` succeeds with no comment; `Accept` requires non-empty comment (400 if missing); transition from Accepted is rejected with 409; concurrent Accepts on the same idea â€” second returns 409 (DbUpdateConcurrencyException â†’ mapped). As Submitter â€” same endpoint returns 403 (FR-005, SC-005)
...
- [ ] T082 [P] [US2] Create `Decision` entity in `backend/src/InnovatEpam.Portal.Domain/Decisions/Decision.cs` per data-model Â§6; constructor enforces "comment required for Accept/Reject"
...
## Phase 5: User Story 3 â€” Browse and filter the catalog (Priority: P3)
...
- [ ] T096 [US3] Extend `IdeaService.ListAsync` to support `status`, `categoryCode`, `submitterId`, `sort` (default `-createdAt`), `page`, `pageSize` â‰¤ 100; ensure index `ix_idea_status_created_at` is used (data-model Â§4) (depends on T063)
...
## Phase 6: Notifications & Polling (supports US2 â†’ US1 feedback loop)

**Purpose**: Implement FR-022 (in-portal notification on status change) discovered in research R14 and tested by M2.10.

- [ ] T101 [P] Create `Notification` entity in `backend/src/InnovatEpam.Portal.Domain/Notifications/Notification.cs` per data-model Â§8; EF configuration adds the partial unread index
...
- [ ] T107 [P] Add `Seed` static method to `InnovatEpam.Portal.Infrastructure` invoked at `Program.cs` startup that seeds the Admin user from `Seed:AdminEmail` / `Seed:AdminPassword` config when no Admin exists (R15, quickstart Â§1.3)
...
- [ ] T110 [P] Write `frontend/tests/unit/lib/date.test.ts` covering `formatIdeaDate`, `formatIdeaDateTime`, `formatRelative` (R10, X4.1â€“X4.3); write a custom ESLint rule or repo-level grep CI step that fails the build if `toLocaleDateString(` or `new Date().toString()` appear in `frontend/src/**/*.tsx` (X4.4)
...
- [ ] T114 [P] Add `dotnet list package --vulnerable --include-transitive` and `npm audit --production` as required CI gates (T017) â€” fail the build on High/Critical (Constitution Dev Workflow gate #5)
...
- [ ] T116 Run the full quickstart Part 2 checklists (M1, M2, M3, X1, X2, X3, X4) end-to-end against a freshly migrated database; record results in the release ticket per quickstart Â§"Sign-off"
...
- **Phase 2 Foundational** â†’ depends on Phase 1; **blocks all stories**.
- **Phase 3 (US1)** â†’ depends on Phase 2.
- **Phase 4 (US2)** â†’ depends on Phase 2 + the `Idea` aggregate from Phase 3 (`T055`, `T058`, `T076`).
- **Phase 5 (US3)** â†’ depends on Phase 2 + the `IdeasListPage`/`IdeaService.ListAsync` from Phase 3 (`T063`, `T075`).
- **Phase 6 Notifications** â†’ depends on Phase 4 (`T085`).
- **Phase 7 Polish** â†’ depends on all desired user stories being complete.

### User Story Independence

- US1 is fully self-contained â€” completing only Phase 1 â†’ 2 â†’ 3 yields a demoable MVP.
- US2 layers on top of US1 (Admin needs ideas to decide on) but is testable without US3.
- US3 is a presentation-layer enhancement of US1 + US2 â€” neither blocks it.
...
- Domain/data tasks â†’ application services â†’ API controllers â†’ frontend wiring.
...
- All Phase 2 frontend foundation tasks (T038â€“T047) parallelise with backend foundation tasks (T019â€“T037).
...
- Polish tasks T107â€“T115 are essentially all [P].

### Parallel Example: User Story 1

```text
After Foundational checkpoint, fan out:
  Dev A: T054 + T055 + T058 (domain/EF) â†’ T063 (IdeaService) â†’ T066 (IdeasController)
  Dev B: T056 + T057 + T064 (attachment service) â†’ T067 (AttachmentsController)
  Dev C: T070 + T071 + T072 (auth FE) â†’ T077 (logout)
...
1. Phase 1 Setup â†’ repo scaffolds and CI green.
2. Phase 2 Foundational â†’ DB migrated, auth pipeline, RFC 7807, OpenAPI, SPA shell.
3. Phase 3 US1 â†’ submit + list + auth.
4. **STOP and run quickstart checklist M1**. If green, demo the MVP.

### Incremental Delivery

1. Setup + Foundational â†’ foundation ready.
2. + US1 â†’ MVP shippable; checklist M1.
3. + US2 â†’ evaluation workflow live; checklist M2.
4. + US3 â†’ catalog browse polished; checklist M3.
5. + Notifications + Polish â†’ release-ready; checklists X1â€“X4 + sign-off.

### Parallel Team Strategy

Once Phase 2 is complete, three streams can ship in parallel:
- Stream A: US1 â†’ US3 (idea capture and catalog).
- Stream B: US2 â†’ Notifications (admin workflow + feedback loop).
...
- After each task or logical group, commit using the constitution-aligned compliance line in the message (e.g., `[US1] add IdeaService â€” complies with Principles I, II`).
